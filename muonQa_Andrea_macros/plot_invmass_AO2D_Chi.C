...
void FitJPsi(TH1* mass_all, TString label, Style_t lineStyle, Width_t lineWidth)
{
  ...
  fit_total_mass_all->Draw();
  fit_bg_mass_all->Draw(); 
  ...
  TPaveText* text;
  if (lineStyle == 1) {
    text = new TPaveText(3.4, peak * 0.7, 4.05, peak);
...
void skeleton_plot_invmass(TString fName, TString label, Style_t lineStyle, Width_t lineWidth)
{
  ...
  if (lineStyle == 1) { invmassMuonTracks1->Draw("E"); 
  } else if (lineStyle == 3) { invmassMuonTracks1->Draw("SAME E"); }
